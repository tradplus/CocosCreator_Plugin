...
@interface TPCOfferwall()<TradPlusADOfferwallDelegate>

@property (nonatomic,strong)TradPlusAdOfferwall *offerwall;
@end

@implementation TPCOfferwall
...
- (void)showAdWithSceneId:(nullable NSString *)sceneId
{
    ...
    [self.offerwall showAdFromRootViewController:[TPCPluginUtil viewController] sceneId:sceneId];
}
...
{
    return [NSString stringWithFormat:@"window.TradPlusOfferwall.%@",event];
}
...
- (void)tpOfferwallGetCurrencyBalance:(NSDictionary *)response error:(NSError *)error
{
    ...
    if(error == nil)
    {
        ...
        NSString *eventNam = [self eventName:@"currencyBalanceSuccess"];
    ...
    else
    {
        ...
        NSString *msg = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSString *eventNam = [self eventName:@"currencyBalanceFailed"];
...
- (void)tpOfferwallSpendCurrency:(NSDictionary *)response error:(NSError *)error
{
    ...
    if(error == nil)
    {
        ...
        NSData *data = [NSJSONSerialization dataWithJSONObject:dic options:0 error:nil];
        NSString *msg = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSString *eventNam = [self eventName:@"spendCurrencySuccess"];
        [TradplusSdkPlugin callbackWithEventName:eventNam adUnitID:self.offerwall.unitID adInfo:nil error:nil exp:@{@"amount":@(amount),@"msg":msg}];
    }
    else
    {
        NSData *data = [NSJSONSerialization dataWithJSONObject:response options:0 error:nil];
        NSString *msg = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSString *eventNam = [self eventName:@"spendCurrencyFailed"];
        [TradplusSdkPlugin callbackWithEventName:eventNam adUnitID:self.offerwall.unitID adInfo:nil error:nil exp:@{@"msg":msg}];
    }
}
...
- (void)tpOfferwallAwardCurrency:(NSDictionary *)response error:(NSError *)error
{
    ...
    if(error == nil)
    {
        ...
        NSData *data = [NSJSONSerialization dataWithJSONObject:dic options:0 error:nil];
        NSString *msg = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSString *eventNam = [self eventName:@"awardCurrencySuccess"];
        [TradplusSdkPlugin callbackWithEventName:eventNam adUnitID:self.offerwall.unitID adInfo:nil error:nil exp:@{@"amount":@(amount),@"msg":msg}];
    }
    else
    {
        NSData *data = [NSJSONSerialization dataWithJSONObject:response options:0 error:nil];
        NSString *msg = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSString *eventNam = [self eventName:@"awardCurrencyFailed"];
        [TradplusSdkPlugin callbackWithEventName:eventNam adUnitID:self.offerwall.unitID adInfo:nil error:nil exp:@{@"msg":msg}];
    }
}
